...
public fun get_emitter(self:&SwapOrderFlat):&vector<u8>{
    &self.emitter
}
...
public fun get_to_token(self:&SwapOrderFlat):vector<u8> {
    self.to_token
}
...
public fun get_token(self:&SwapOrderFlat):vector<u8> {
    return self.token
}

public fun get_amount(self:&SwapOrderFlat):u128 {self.amount}
public fun emit(self:SwapOrderFlat){
    event::emit(self)
}

public(package) fun deduct_min_receive(self:&mut SwapOrderFlat,amount:u128){
    self.min_receive=self.min_receive-amount

}

public(package) fun deduct_amount(self:&mut SwapOrderFlat,amount:u128){
...
public fun encode(self:&SwapOrderFlat):vector<u8>{
     let mut list=vector::empty<vector<u8>>();
          ...
          vector::push_back(&mut list,encoder::encode(&self.get_creator()));
           ...
           vector::push_back(&mut list,encoder::encode(&self.get_token()));
           vector::push_back(&mut list,encoder::encode_u128(self.get_amount()));
           vector::push_back(&mut list,encoder::encode(&self.get_to_token()));
...
public fun decode(bytes:&vector<u8>):SwapOrderFlat{
       let decoded=decoder::decode_list(bytes);
        ...
        let src_nid= decoder::decode_string(decoded.borrow(2));
         ...
         let creator= *vector::borrow(&decoded,4);
         let destination_address=*vector::borrow(&decoded,5);
         let token= *vector::borrow(&decoded,6);
         let amount= decoder::decode_u128(decoded.borrow(7));
         let to_token=*vector::borrow(&decoded,8);
