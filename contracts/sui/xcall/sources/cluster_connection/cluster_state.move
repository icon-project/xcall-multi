module xcall::cluster_state_optimized {
    ...
    public struct State has store{ 
        ...
        receipts: Table<ReceiptKey, bool>,
        conn_sn: u128,
        balance: Balance<SUI>,
        validators: vector<vector<u8>>,
    ...
    public(package) fun create(ctx: &mut TxContext): State {
        State {
            message_fee: vec_map::empty<String, u64>(),
            response_fee: vec_map::empty<String, u64>(),
            conn_sn: 0,
            receipts: table::new<ReceiptKey, bool>(ctx),
    ...
    public(package) fun check_save_receipt(self: &mut State, net_id: String, sn: u128) {
        ...
        table::add(&mut self.receipts, receipt_key, true);
    }

    public(package) fun get_receipt(self: &State, net_id: String, sn: u128): bool {
        let receipt_key = ReceiptKey { nid: net_id, conn_sn: sn };
        self.receipts.contains(receipt_key)
    }
    ...
    #[test_only]
    public(package) fun create_state(ctx: &mut TxContext):State{
        State {
            message_fee: vec_map::empty<String, u64>(),
            response_fee: vec_map::empty<String, u64>(),
            conn_sn: 0,
...
module xcall::cluster_state_tests {
    use xcall::cluster_state_optimized::{State, get_validators, get_validator_threshold, set_validator_threshold, set_validators, verify_signatures};
    use sui::test_scenario::{Self, Scenario};

    #[test]
    fun test_add_validator(): State {
        let mut scenario = test_scenario::begin(@0xBABE);
        let ctx = test_scenario::ctx(&mut scenario);
        let mut state = xcall::cluster_state_optimized::create_state(ctx);
    ...
    fun test_get_fee(): State {
        ...
        let ctx = test_scenario::ctx(&mut scenario);
        let mut state = xcall::cluster_state_optimized::create_state(ctx);        
        xcall::cluster_state_optimized::set_fee(&mut state, b"net1".to_string(), 100, 50, @0xadd);
        xcall::cluster_state_optimized::set_fee(&mut state, b"net2".to_string(), 200, 100, @0xadd);

        let fee_without_response = xcall::cluster_state_optimized::get_fee(&state, &b"net1".to_string(), false);
        assert!(fee_without_response == 100);

        let fee_with_response = xcall::cluster_state_optimized::get_fee(&state, &b"net1".to_string(), true);
    ...
    fun test_update_fee(): State { 
        ...
        let ctx = test_scenario::ctx(&mut scenario);
        let mut state = xcall::cluster_state_optimized::create_state(ctx);        
        xcall::cluster_state_optimized::set_fee(&mut state, b"net1".to_string(), 200, 100, @0xadd);

        let fee = xcall::cluster_state_optimized::get_fee(&state, &b"net1".to_string(), true);
        assert!(fee == 300); // 200 message_fee + 100 response_fee

        // Update the fee
        xcall::cluster_state_optimized::set_fee(&mut state, b"net1".to_string(), 300, 200, @0xadd);
        let updated_fee = xcall::cluster_state_optimized::get_fee(&state, &b"net1".to_string(), true);
    ...
    fun test_receipts(): State {
        ...
        let ctx = test_scenario::ctx(&mut scenario);
        let mut state = xcall::cluster_state_optimized::create_state(ctx);
        let sn = xcall::cluster_state_optimized::get_next_conn_sn(&mut state);
        
        xcall::cluster_state_optimized::check_save_receipt(&mut state, b"net1".to_string(), sn);
        let receipt_exists = xcall::cluster_state_optimized::get_receipt(&state, b"net1".to_string(), sn);
    ...
    fun test_verify_signatures_less_than_threshold(): State {
        ...
        xcall::cluster_state_optimized::verify_signatures(&state,src_net_id, conn_sn, msg, dst_net_id, signatures);
        state
    }
