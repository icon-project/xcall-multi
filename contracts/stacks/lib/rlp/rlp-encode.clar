        ...
        (if (is-eq (len encoded) u5)  ;; Empty string is type byte + 4 length bytes
            ...
            (let (
                ...
                (if (> encoded-length u55)
                    (let (
                        (length-byte (unwrap-panic (element-at? (unwrap-panic (to-consensus-buff? encoded-length)) u16)))
                        (prefix (concat 0xb8 length-byte))
                    )
...
(define-private (encode-buff-arr (objects (list 500 (buff 1024))))
    (fold concat-buff (map encode-buff objects) 0x)
)
